...
int main()
{
    ...
    Shader shader("../Shaders/5_4_normalMap.vs.glsl", "../Shaders/5_4_normalMap.fs.glsl");

    // load textures
    // -------------
    unsigned int diffuseMap = loadTexture("../resources/textures/brickwall.jpg", true);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        model = glm::rotate(model, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show normal mapping from multiple directions        shader.setVec3("lightPos", lightPos);
