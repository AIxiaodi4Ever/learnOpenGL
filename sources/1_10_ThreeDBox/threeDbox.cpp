...
int main()
{
	...
	// ---------------------------------------------------------------
	Shader ourShader("../Shaders/threeDbox.vs", "../Shaders/threeDbox.fs");
	...
	glm::vec3 cubePositions[] = {
		...
		glm::vec3(1.5f, 0.2f, -1.5f),	//9
		glm::vec3(-1.3f, 1.0f, -1.5f)	//10
	};
	// vertex array object
	...
	while (!glfwWindowShouldClose(window)) {
		...
		projection = glm::perspective(glm::radians(60.0f), (float)SCR_WIDTH  / (float)SCR_HEIGHT, 0.1f, 100.f);
		...
		for (int i = 0; i < 10; ++i) {
			...
			model = glm::rotate(model, glm::radians(angle), glm::normalize(glm::vec3(1.0f + i, 0.3f + i, 0.5f + i)));
