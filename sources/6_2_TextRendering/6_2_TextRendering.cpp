...
int main()
{
    ...
    // -------------------------
    Shader pbrShader("../Shaders/5_11_pbr_full.vs.glsl", "../Shaders/5_11_pbr_full.fs.glsl");
    Shader equirectangularToCubemapShader("../Shaders/5_11_cubemap.vs.glsl", "../Shaders/5_11_equirectangular_to_cubemap.fs.glsl");
    ...
    Shader textShader("../Shaders/6_2_text.vs.glsl", "../Shaders/6_2_text.fs.glsl");
    ...
    // configure charVAO/VBO for texture quads
    // -----------------------------------
    ...
    // render loop
    // -----------
    // 1. Disable GL_CULL_FACE unless background pass would render nothing.
    // 2. Put GL_BLEND before render loop, unless pre-filter or convolution would be invalid.
    // glEnable(GL_CULL_FACE);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    string framerateInfo = "Framerate is: ";
    int framerate = 0;
    while (!glfwWindowShouldClose(window))
    {
        ...
        ++cumulateFrame;
        if (cumulateTime >= 1.0) {
            framerate = float(cumulateFrame) / cumulateTime;
            cumulateFrame = 0;
            cumulateTime = 0.0;
        }
        ...
        glm::mat4 texProjection = glm::ortho(0.0f, static_cast<float>(SCR_WIDTH), 0.0f, static_cast<float>(SCR_HEIGHT));
        textShader.use();
        textShader.setMat4("projection", texProjection);
        RenderText(textShader, "This is sample text", 25.0f, 25.0f, 0.3f, glm::vec3(0.5, 0.8f, 0.2f));
        RenderText(textShader, framerateInfo + to_string(framerate), 25.0f, 50.0f, 0.3f, glm::vec3(0.5, 0.8f, 0.2f));
        RenderText(textShader, "https://github.com/AIxiaodi4Ever/learnOpenGL", 25.0f, 680.0f, 0.5f, glm::vec3(0.3, 0.7f, 0.9f));
