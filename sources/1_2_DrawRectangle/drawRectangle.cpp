...
int main()
{
	...
	// location of attribute, numbers of value per attribute, data type, stride, offset
	// the stride can set to 0 when the data is tightly packed to let OpenGL determine the stride.
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);

	/* this is allowed, the call to glVertexAttribPointer registered VBO as the vertex attribute's bound vertex buffer object so afterwards we can safelt unbind */
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);  don't like VBO, we can't unbind EBO before VAO is unbinded.
